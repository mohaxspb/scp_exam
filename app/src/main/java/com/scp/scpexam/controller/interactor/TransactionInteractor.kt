        ...
        apiClient.getNwQuizTransactionList()
            ...
            .doOnSuccess { nwTransactionList ->
                nwTransactionList.forEach { nwQuizTransaction ->
                    val finishedLevel =
                        appDatabase.finishedLevelsDao().getByQuizId(nwQuizTransaction.quizId!!)
                    val quizTransactionFromBd = appDatabase.transactionDao()
                        .getOneByQuizIdAndTransactionType(
                            nwQuizTransaction.quizId!!,
                            nwQuizTransaction.quizTransactionType
                        )
...
//                                Timber.d("Inserted transaction :%s", quizTransactionFromBd)
                        ...
                        if (quizTransactionFromBd.externalId == null) {
                            appDatabase.transactionDao().updateQuizTransactionExternalId(
                                quizTransactionFromBd.id!!,
                                nwQuizTransaction.id
                            )
//                                    Timber.d("Inserted transaction when ExtId == null :%s", quizTransactionFromBd)
    ...
    private fun syncScoreWithServer() =
        Maybe.fromCallable {
            val updateSyncTransaction =
                appDatabase.transactionDao().getOneByTypeNoReactive(TransactionType.UPDATE_SYNC)
            Timber.d(
                "appDatabase.transactionDao().getOneByTypeNoReactive(TransactionType.UPDATE_SYNC) : %s",
                appDatabase.transactionDao().getOneByTypeNoReactive(TransactionType.UPDATE_SYNC)
            )
            ...
            .map { levelsAvailableTrue ->
                ...
                levelsAvailableTrue.forEach { levelAvailableFinishedLevel ->
                    ...
                    if (levelAvailableFinishedLevel.nameRedundantCharsRemoved
                        && appDatabase.transactionDao().getOneByQuizIdAndTransactionType(
                            quizId,
                            TransactionType.NAME_CHARS_REMOVED
                        ) == null
                        && appDatabase.transactionDao().getOneByQuizIdAndTransactionType(
                            quizId,
                            TransactionType.NAME_CHARS_REMOVED_MIGRATION
                        ) == null
                    ) {
                        finishedLevelsToTransactions.add(
                            quizTransactionForMigration(
                                quizId,
                                TransactionType.NAME_CHARS_REMOVED_MIGRATION
                            )
                        )
                    ...
                    if (levelAvailableFinishedLevel.numberRedundantCharsRemoved
                        ...
                        && appDatabase.transactionDao().getOneByQuizIdAndTransactionType(
                            quizId,
                            TransactionType.NUMBER_CHARS_REMOVED_MIGRATION
                        ) == null
                    ) {
                        finishedLevelsToTransactions.add(
                            quizTransactionForMigration(
                    ...
                    if (levelAvailableFinishedLevel.scpNameFilled
                        ...
                        && appDatabase.transactionDao().getOneByQuizIdAndTransactionType(
                            quizId,
                            TransactionType.NAME_ENTERED_MIGRATION
                        ) == null
                    ) {
                        finishedLevelsToTransactions.add(
                            quizTransactionForMigration(
                    ...
                    if (levelAvailableFinishedLevel.scpNumberFilled
                        ...
                        && appDatabase.transactionDao().getOneByQuizIdAndTransactionType(
                            quizId,
                            TransactionType.NUMBER_ENTERED_MIGRATION
                        ) == null
                    ) {
                        finishedLevelsToTransactions.add(
                            quizTransactionForMigration(
                    ...
                    if (appDatabase.transactionDao().getOneByQuizIdAndTransactionType(
                            quizId,
                            TransactionType.LEVEL_ENABLE_FOR_COINS
                        ) == null
                        && appDatabase.transactionDao().getOneByQuizIdAndTransactionType(
                            quizId,
                            TransactionType.LEVEL_AVAILABLE_MIGRATION
                        ) == null
                    ) {
                        finishedLevelsToTransactions.add(
                            quizTransactionForMigration(
                    ...
                    .flatMapSingle {
                        apiClient
                            .addTransaction(
                                ...
                                typeTransaction = appDatabase.transactionDao()
                                    .getOneById(it).transactionType,
                                coinsAmount = appDatabase.transactionDao()
                                    .getOneById(it).coinsAmount
                            )
                            .doOnSuccess { nwQuizTransaction ->
